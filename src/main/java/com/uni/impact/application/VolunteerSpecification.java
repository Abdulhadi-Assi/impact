...
public class VolunteerSpecification {
    ...
    /**
     * Filter applications by status only (حالة الطلبات)
     */
    public static Specification<Application> withStatus(ApplicationStatus status) {
    ...
    /**
     * Filter applications by college ID
     */
    public static Specification<Application> withCollegeId(Long collegeId) {
        return (root, query, criteriaBuilder) -> {
    ...
    /**
     * Filter applications by campaign ID
     */
    public static Specification<Application> withCampaignId(Long campaignId) {
        return (root, query, criteriaBuilder) -> {
            Predicate predicate = criteriaBuilder.conjunction();
            if (campaignId != null) {
                Join<Object, Object> campaignJoin = root.join("campaign");
                predicate = criteriaBuilder.and(predicate,
                    criteriaBuilder.equal(campaignJoin.get("campaignId"), campaignId));
            }
            return predicate;
        };
    }
     ...
     */
    public static Specification<Application> withStudentId(Long studentId) {
        return (root, query, criteriaBuilder) -> {
            Predicate predicate = criteriaBuilder.conjunction();
            if (studentId != null) {
                Join<Object, Object> studentJoin = root.join("student");
                predicate = criteriaBuilder.and(predicate,
                    criteriaBuilder.equal(studentJoin.get("userId"), studentId));
            }
            return predicate;
        };
    }
